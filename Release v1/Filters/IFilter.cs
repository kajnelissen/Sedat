...
namespace Filters
{
    ...
    public abstract class IFilter
    {
        ...
        public List<string> OrderToString(int id)
        {
            for (int index = 0; index < input.Count; index++)
            {
                if (input.ElementAt(index).OrderId == id)
                {
                    List<string> componenten = new List<string>();
                    for (int index2 = 0; index2 < input.ElementAt(index).Components.Count; index2++)
                    {
                        componenten.Add(input.ElementAt(index).Components[index2].ToString());
                    }
                    return componenten;
                }
            }
